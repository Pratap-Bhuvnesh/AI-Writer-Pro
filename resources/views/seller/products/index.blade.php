<x-seller-layout title="Products">
    <section class="heading">
        <div>
            <h1>Seller Products</h1>
            <p class="copy">Create and manage products owned by your seller account.</p>
        </div>
    </section>
    ...
    <section class="panel" style="margin-bottom:18px;">
        <form method="POST" action="{{ route('seller.products.store') }}">
            ...
            <div class="form-grid">
                ...
                <div class="field"><label>Discount</label><input type="number" step="0.01" min="0" name="discount_price"></div>
                <div class="field"><label>Stock</label><input type="number" min="0" name="stock_quantity" value="10" required></div>
            </div>
        ...
        <table>
            ...
            <tbody>
                @forelse ($products as $product)
                    @php $variant = $product->variants->first(); $inventory = $variant?->inventory; $available = $inventory ? $inventory->stock_quantity - $inventory->reserved_quantity : 0; @endphp
                    <tr>
                        <td><strong>{{ $product->name }}</strong><br><span class="copy">{{ $product->brand ?? 'No brand' }}</span></td>
                        <td>{{ $product->category?->name ?? 'N/A' }}</td>
                        <td>${{ number_format((float) ($variant?->discount_price ?? $variant?->price ?? 0), 2) }}</td>
                        <td>{{ $available }}</td>
                        <td>
                            <details>
                                <summary class="button secondary" style="display:inline-flex;">Edit</summary>
                                <form method="POST" action="{{ route('seller.products.update', $product) }}" style="margin-top:12px; min-width:520px;">
                                    @csrf @method('PUT')
                                    <div class="form-grid" style="grid-template-columns:repeat(2,minmax(0,1fr));">
                                        ...
                                        <div class="field"><label>Category</label><select name="category_id">@foreach($categories as $category)<option value="{{ $category->id }}" @selected($product->category_id == $category->id)>{{ $category->name }}</option>@endforeach</select></div>
                            ...
                            <form method="POST" action="{{ route('seller.products.delete', $product) }}" style="margin-top:8px;">
                                @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="5">No seller products found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </section>
